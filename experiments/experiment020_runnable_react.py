"""
Runnable React Experiment

Same as experiment019 but goes further: after generation + tsc validation,
scaffolds a full Vite project (index.html, main.tsx) and starts the dev server.
You can open it in a browser to see the actual running app.
...
### Result
    ...
    Vite dev server: RUNS — app visible in browser at localhost:5173

### Summary of the 20-experiment journey

    Exp001  Recursive tree, no contracts          — proof of concept
    Exp002  Contracts (typed interfaces)           — no duplication
    Exp003  Async spawning                         — parallel children
    Exp004  Inline file routing                    — race condition
    Exp005  Deferred routing                       — consistent paths
    Exp006  Filesystem routing                     — writes to disk
    Exp007  Structure-aware prompts                — multi-file output
    Exp008  React specialization (Haiku)           — component tree
    Exp009  React with Sonnet                      — quality jump
    Exp010  Timed (baseline)                       — 356s, 247K tokens
    Exp011  Pipelined routing                      — no improvement
    Exp012  Self-routing                           — 188s, agents write own files
    Exp013  Light contracts                        — 130s, shorter specs
    Exp014  Forked agents                          — 109s, context inheritance
    Exp015  Generic agent                          — 54s, one system prompt
    Exp016  Haiku rerun                            — 24s wall clock
    Exp017  Sonnet rerun                           — confirms caching
    Exp018  Two-wave fork                          — slower, unnecessary
    Exp019  Validated (tsc)                         — 0 errors first try
    Exp020  Runnable (Vite)                        — runs in browser

### Winning Architecture

    - Generic agent: one system prompt, one run_agent function
    - Forked context: children inherit parent's full conversation
    - Single-wave fork: all children spawned at once, parallel
    - Architect root: plans + delegates via user message, not system prompt
    - Self-routing: agents write directly to their assigned file_path
    - Prompt caching: shared parent prefix cached across children
    - Validation: tsc --noEmit confirms correctness

### Performance (exp015 architecture)

    Generation:    ~55s
    Validation:     ~2s
    API calls:     ~22
    Tokens:        ~8K + ~80K cache reads
    Cost:          ~$0.30
    TS errors:     0
"""
...
<html lang="en">
  <head>
    ...
    <title>Morph Generated App</title>
  </head>
  <body>
    <div id="root"></div>
...
def validate_project(project_root: str) -> tuple[bool, str, float]:
    ...
    # Write main.tsx if it doesn't exist
...
def main():
    ...
    project_root = tempfile.mkdtemp(prefix="morph_exp020_")
    total_start = time.monotonic()

    print("=" * 60)
    print("RUNNABLE REACT EXPERIMENT")
    print("=" * 60)
    print(f"\nRoot task: {root_task}")
    ...
    # Phase 3: Start Vite dev server
    if success:
        print("\n" + "=" * 60)
        print("STARTING DEV SERVER")
        print("=" * 60)
        print(f"\n  Starting Vite at {project_root}...")
        print("  Open http://localhost:5173 in your browser.")
        print("  Press Ctrl+C to stop.\n")

        executor.shutdown(wait=False)

        # Run vite in foreground so user can Ctrl+C
        try:
            subprocess.run(
                ["npx", "vite", "--host"],
                cwd=project_root,
        ...
        print("\n  Skipping dev server — tsc failed.")
        executor.shutdown(wait=False)
