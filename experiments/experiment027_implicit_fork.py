"""
Implicit Fork Experiment

The agent has NO fork/spawn/delegate tools. It just writes code. The system
watches for undefined component references in the code and auto-forks child
agents to implement them.

When the root agent writes <SearchBar query={query} onChange={setQuery} />,
the system:
1. Detects SearchBar is not defined in any file
2. Extracts the props from usage (query: string, onChange: function)
3. Shows a skeleton in the browser
4. Forks a child agent with the parent's context + "implement SearchBar"
5. Child writes the component, skeleton morphs into real thing

The agent tree emerges from the code structure, not from explicit orchestration.
...
### Timing Summary
    ...
    Built in:         133s
    tsc --noEmit:     PASS
    Root API calls:   ~4 (types, utils, hook, App.tsx, done)
    Auto-forked:      6 agents (5 from root + 1 recursive from ProductGrid)

### What Happened

    1. Root writes App.tsx importing SearchBar, SortControls, ProductGrid,
       LoadingSpinner, ErrorMessage (agent invented ErrorMessage on its own)
    2. System detects 5 undefined imports -> writes 5 stubs instantly
       -> browser shows 5 "Building..." placeholders
    3. System auto-forks 5 child agents in parallel
    4. ProductGrid imports ProductCard -> system detects it -> stubs it
       -> auto-forks a 6th agent (recursive, depth 2)
    5. All children replace their stubs with real implementations
    6. tsc PASS

### Auto-Emerged Agent Tree

    root (wrote App.tsx — no fork calls)
      SearchBar        <- auto-detected from import
      SortControls     <- auto-detected from import
      ProductGrid      <- auto-detected from import
        ProductCard    <- recursive: auto-detected from ProductGrid's import
      LoadingSpinner   <- auto-detected from import
      ErrorMessage     <- auto-detected (agent invented this on its own!)

### Key Findings

- Zero orchestration tools. Agent has only read_file, append_to_file,
  replace_text, done. The tree emerges from import analysis.
- Recursive auto-fork works: ProductGrid -> ProductCard at depth 2.
- Agent invented ErrorMessage without being asked — the system just
  detected the undefined import and auto-forked for it.
- Stubs prevent Vite crashes: placeholder files created instantly when
  undefined imports are detected, before child agents even start.
- Browser shows "Building X..." placeholders that morph into real
  components as children finish.
- The simplest agent definition yet: no contracts, no fork_batch,
  no wait_all. Just write code naturally.
"""
...
"""
    ...
    def _run_agent(self, node: AgentNode, user_message: str, depth: int = 0) -> list[dict]:
        """Run an agent. Returns list of undefined imports found in written files."""
        indent = "  " * depth
        ...
        while not is_done:
            ...
            for block in tool_use_blocks:
                if block.name in ("read_file", "append_to_file", "replace_text"):
                    ...
                    self._print(f"{indent}  [{block.name}] {fp[:50]}")
                    ...
                    if block.name in ("append_to_file", "replace_text"):
                        ...
                        if os.path.exists(written_path):
                            ...
                            # Write stubs immediately so Vite doesn't crash
