"""
Image Editor Experiment

Stress test: use the exp022 architecture (persistent agents, no verify, forked
context) to build a web-based image editor — canvas, toolbar, layers panel,
undo/redo, drawing tools. The biggest task we've tried so far.
"""
...
def run_agent_turn(
    ...
    while not is_done:
        ...
        for task_id, fork_input in pending_forks:
            ...
            child_task = fork_input.get("task", f"Implement {child_name} and write to {child_path}.")
...
def main():
    root_task = (
        "Build a web-based image editor in React, like a simple Photoshop. Features:\n"
        "- A main canvas (HTML5 Canvas) that displays and edits images\n"
        "- A toolbar on the left with tools: brush, eraser, rectangle, circle, line, text, color picker\n"
        "- A layers panel on the right showing layer stack with visibility toggles and layer ordering\n"
        "- A top menu bar with: New, Open (file upload), Save (download as PNG), Undo, Redo\n"
        "- A properties panel below the toolbar showing current tool settings (brush size, color, opacity)\n"
        "- A custom hook useCanvas that manages canvas state, drawing operations, and layer compositing\n"
        "- A custom hook useHistory for undo/redo with canvas snapshots\n"
        "- TypeScript types for Tool, Layer, CanvasState, DrawOperation\n"
        "- The canvas should support freehand drawing with the brush tool\n"
    ...
    root_message = (
        ...
        "1. Plan the structure carefully. Write ALL shared types first (src/types/index.ts).\n"
        "2. Fork a child for each component, each hook, and App.tsx.\n"
    ...
    project_root = tempfile.mkdtemp(prefix="morph_exp023_")

    print("=" * 60)
    print("IMAGE EDITOR EXPERIMENT")
    print("=" * 60)
