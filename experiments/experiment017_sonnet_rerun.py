"""
Sonnet Rerun Experiment

Rerun of experiment015 (generic agent, forked context, architect root) with
claude-sonnet-4-6. Verifying the 8K token / 61K cache read result and checking
if cache behavior is consistent across runs.

## Experiment Results (2026-03-27)

Model: claude-sonnet-4-6
Root task: Product dashboard.
8 agents, 27 API calls, 12K tokens + 103K cache reads.

### Timing Summary

    Total wall clock:     77s
    Total API calls:      27
    Total tokens:         12,088 (3.2K in, 8.9K out)
    Cache read tokens:    102,887
    Cache creation:       28,050

### Comparison

    | Metric         | Exp015 Sonnet | Exp016 Haiku | Exp017 Sonnet |
    |----------------|---------------|--------------|---------------|
    | Wall clock     | 54s           | 24s          | 77s           |
    | Total tokens   | 8K            | 48K          | 12K           |
    | Cache reads    | 61K           | 13K          | 103K          |
    | Input tokens   | 1.4K          | 41K          | 3.2K          |

### Key Findings

- Confirms Sonnet + forking + caching pattern: ~10K real tokens, ~100K cache.
- 54s vs 77s variance is due to root taking 8 calls vs 5 (more scaffolding).
- Cache hits are consistent and high across Sonnet runs (61K and 103K).
- Haiku is faster (24s) but pays full input tokens (41K) due to fewer cache
  hits. Sonnet is slower but cheaper per-token via caching.
- Sticking with Sonnet for quality + cache efficiency.
"""
...
def main():
    ...
    project_root = tempfile.mkdtemp(prefix="morph_exp017_")
    total_start = time.monotonic()

    print("=" * 60)
    print("SONNET RERUN EXPERIMENT")
    print("=" * 60)
    print(f"\nRoot task: {root_task}")
