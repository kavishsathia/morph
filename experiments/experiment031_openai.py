...
3D editor task using OpenAI gpt-5.3-codex via Responses API.
Implicit fork architecture with server-side conversation forking
(previous_response_id). Shimmer stub placeholders. shadcn + Tailwind.

## Experiment Results (2026-03-28)

Model: gpt-5.3-codex (OpenAI Responses API)
Task: 3D object editor.

### Result

    Built in:     42s
    tsc:          PASS
    Auto-forked:  4 (HeaderBar, TransformToolbar, Viewport3D, PropertiesPanel)

### What happened

    1. Root writes types + App.tsx (2 write_file calls, 1 API round)
    2. System detects 4 undefined imports -> 4 shimmer stubs appear instantly
    3. 4 children forked in parallel, each inherits parent context via
       previous_response_id (Responses API server-side fork)
    4. Each child: 1 write_file + done (single API call each)
    5. Shimmer placeholders morph into real components as children finish

### Comparison

    | Model              | Time  | tsc  | Forking              |
    |--------------------|-------|------|----------------------|
    | Groq gpt-oss-120b  | 13.7s | PASS | message copy         |
    | OpenAI Codex       | 42.0s | PASS | server-side (resp ID)|
    | Sonnet (exp027)    | 130s  | PASS | message copy         |

### Key Findings

- Codex is 3x slower than Groq but has server-side conversation forking
  via previous_response_id — children inherit parent context without
  copying messages.
- Codex tried to write everything in one file initially (too capable).
  Had to explicitly instruct decomposition.
- Shimmer stub loaders fill containers and animate while waiting.
- Each child did exactly 1 API call (write_file + done) — minimal.
- Responses API required: clean response ID (no pending tool calls)
  before forking, final tool output flush after done().
"""
...
class AgentNode:
    ...
    last_response_id: str | None = None  # for Responses API context forking


class Morph:
    # Tools for the Responses API
    AGENT_TOOLS = [
        {"type": "function", "name": "read_file", "description": "Read a file.", "parameters": {"type": "object", "properties": {"file_path": {"type": "string"}}, "required": ["file_path"]}},
        {"type": "function", "name": "write_file", "description": "Write content to a file. OVERWRITES the entire file. Creates dirs if needed.", "parameters": {"type": "object", "properties": {"file_path": {"type": "string"}, "content": {"type": "string"}}, "required": ["file_path", "content"]}},
        {"type": "function", "name": "done", "description": "Signal you are finished.", "parameters": {"type": "object", "properties": {}}},
    ]
...
- For icons use lucide-react: import { Search, Star, ChevronDown, Move, RotateCw, Maximize,
  Box, Layers, Settings, Menu, X, Plus, Minus, Eye, EyeOff, Undo, Redo, Sun, Moon } from "lucide-react"
  Use ONLY these icon names. Do NOT guess icon names — if unsure, use Box as a fallback.
...
'''
    ...
    def __init__(self, model="gpt-5.3-codex", project_root=None, max_depth=3, port=5173):
        self.model = model
    ...
    def _write_stub(self, item: dict) -> None:
        ...
        if file_path.endswith(".ts") and "hook" in file_path.lower():
            ...
            # Component stub — shimmer loader that fills its container
            stub = (
    ...
    def _run_agent(self, node: AgentNode, user_message: str, depth: int = 0) -> list[dict]:
        """Run an agent using OpenAI Responses API. Returns undefined imports."""
        indent = "  " * depth
        ...
        # If this node was forked from a parent, use the parent's response ID
        # so the child inherits the full conversation context server-side
        prev_response_id = node.last_response_id
        ...
        while not is_done:
            ...
            if not function_calls:
                # No pending tool calls — safe to fork from this point
                node.last_response_id = response.id
                for item in response.output:
                    if hasattr(item, "text"):
                        preview = (item.text or "")[:200].replace("\n", " ")
                        self._print(f"{indent}  [response] {preview}...")
                break
        ...
        # If we exited with pending tool outputs (e.g. done() was called),
        # send them back to get a clean response we can fork from
        if input_items and prev_response_id:
            ...
            node.last_response_id = final.id
    ...
    def _auto_fork_undefined(self, parent_node: AgentNode, undefined: list[dict], depth: int) -> None:
        ...
        for item in to_fork:
            ...
            child_node = AgentNode(name=child_name, messages=[], last_response_id=parent_node.last_response_id)
    ...
    def build(self, task: str) -> "Morph":
        ...
        message = (
            ...
            "2. src/App.tsx — the main app component (layout + state only)\n\n"
            ...
            "IMPORTANT — DECOMPOSE THE UI:\n"
            "App.tsx should ONLY handle layout and top-level state. For every distinct UI\n"
            "section (toolbar, viewport, sidebar, panel, header, etc.), import a separate\n"
            "component from ./components/. Do NOT implement them — just import and use them\n"
            "with the right props. Other agents will auto-implement any component you reference.\n\n"
            "Example:\n"
            "  import Toolbar from './components/Toolbar';\n"
            "  import Viewport from './components/Viewport';\n"
            "  import PropertiesPanel from './components/PropertiesPanel';\n\n"
            "The more you decompose, the faster the build (components are built in parallel).\n"
            "Write types + App.tsx with write_file. Then done()."
        )
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("OPENAI GPT-5.3-CODEX — 3D EDITOR")
    print("=" * 60)
