...
Same implicit fork architecture as experiment027 but using Groq instead of
Claude Sonnet. Tests faster/cheaper models with the auto-fork pattern.
...
### Models Tested
    ...
    | Model                        | Result                           |
    |------------------------------|----------------------------------|
    | qwen/qwen3-32b              | Tool call JSON escaping failure  |
    | openai/gpt-oss-120b         | 18s, tsc PASS — best result      |
    | meta-llama/llama-4-scout    | tested                           |
    | groq/compound               | tested                           |

### Best Result: openai/gpt-oss-120b

    Built in:         18.4s
    tsc --noEmit:     PASS
    Auto-forked:      5 agents (4 from root + 1 recursive ProductCard)

### Comparison

    | Metric         | Sonnet (exp027) | Groq gpt-oss-120b (exp029) |
    |----------------|-----------------|----------------------------|
    | Wall clock     | 130s            | 18s (7.2x faster)          |
    | tsc            | PASS            | PASS                       |
    | Architecture   | same            | same                       |

### Key Findings

- 7x faster than Sonnet on the same implicit fork architecture.
- Groq's LPU inference speed makes the multi-agent pattern even more
  effective — children complete in ~2-3s each.
- gpt-oss-120b handles tool calling correctly. Qwen 3 32B failed on
  complex JSON escaping in tool call arguments.
- Same architecture, same code, just swap the model + client.
  The implicit fork pattern is model-agnostic.

### Update: shadcn/ui + Tailwind (2026-03-28)

- Added pre-bundled shadcn components (Button, Input, Card, Badge, Separator,
  Skeleton) and Tailwind CSS with shadcn design tokens.
- Agent uses Tailwind classes and shadcn components instead of inline styles.
- Much better visual output — proper design system, consistent spacing,
  hover states, focus rings, all from the component library.
- Had to restrict the prompt to ONLY list available components — the model
  tried to import ScrollArea, Tabs, etc. which weren't scaffolded.
- Needed full tailwind.config.js with shadcn color definitions for
  CSS variable-based colors (border-border, bg-background, etc.).
- The key insight: giving the agent a design system produces better output
  with fewer tokens than asking it to invent styles from scratch.
"""
...
'''
    ...
    def build(self, task: str) -> "Morph":
        ...
        message = (
            ...
            "Write types, utils, hook with write_file. Replace App.tsx with write_file. Then done()."
        )
