...
custom hook for data fetching/filtering, TypeScript types, mock data.
Max depth: 2, 15 modules in pre-order queue (with duplicates).

### Project Layout

    src/
      App.tsx                                        — root component
      types/index.ts                                 — Product type
      utils/mockProducts.ts                          — 10 mock products
      hooks/useProductSearch.ts                      — search + sort logic
      components/
        ProductCard/ProductCard.tsx + index.ts        — card with name, price, desc
        SearchBar/SearchBar.tsx + index.ts            — controlled search input
        SortButtons/SortButtons.tsx + index.ts        — name/price sort toggles
        LoadingSpinner/LoadingSpinner.tsx + index.ts  — conditional spinner
        ProductDashboard/ProductDashboard.tsx + index.ts — orchestrates everything

### Observations

- Proper React project layout: components in folders with barrel exports,
  hooks in /hooks, types in /types, utils in /utils.
- Contracts contained both props interfaces and behavioral specs — agents
  knew what to render AND how to behave (state, effects, events).
- Component tree mirrors the actual React tree:
  App -> ProductDashboard -> SearchBar/SortButtons/ProductCard.
- TypeScript throughout: React.FC, typed props, proper interfaces.
- Duplicates problem: ProductDashboard re-spawned children that already
  existed (SearchBar, LoadingSpinner, etc.) at depth 2, causing double
  content in some files. Need deduplication in the pre-order queue.
- useProductSearch appended twice with slightly different implementations.
- Router didn't check existing content well enough before appending.
- Fix for next experiment: deduplicate queue by name before routing.
"""
...
def main():
    ...
    project_root = tempfile.mkdtemp(prefix="morph_exp008_")

    print("=" * 60)
    print("REACT AGENT TREE EXPERIMENT")
    print("=" * 60)
    print(f"\nRoot task: {root_task}")
