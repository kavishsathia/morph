"""
Persistent Agent Tree Experiment

After initial generation, the agent tree stays alive. Each agent's conversation
history is preserved. You can send modification requests to the root, which
decides whether to handle it or delegate to child agents. Children can further
delegate or solve it themselves.

This enables iterative development — generate, validate, modify, repeat.
...
### Initial Generation
    ...
    Agent tree:       architect (15 messages, 6 children)
                        child-product-card (14 messages)
                        child-search-bar (14 messages)
                        child-sort-controls (14 messages)
                        child-loading-spinner (14 messages)
                        child-use-products (12 messages)
                        child-app (12 messages)

### Modification: "add a dark mode toggle button"

    Modification time: 68s
    tsc --noEmit:      PASS (0 errors)

    What happened:
    1. Architect updated src/types/index.ts (added Theme type)
    2. Delegated to ALL 5 child agents in parallel:
       - Apptsx: added useState for theme, toggle button, CSS vars
       - SearchBar: added dark prop, conditional styling
       - SortControls: added dark prop, conditional styling
       - ProductCard: added dark prop, conditional styling
       - LoadingSpinner: added dark prop, conditional styling
    3. Each child read its own file, applied replace_text modifications
    4. Architect waited for all, verified the files
    5. tsc PASS — 0 errors after modification

    The app runs in the browser with working dark mode toggle.

### Key Findings

- Persistent agent tree enables iterative development. Agents remember
  what they wrote and can make targeted edits.
- Modification delegated to 5 children in parallel — same fork pattern
  as initial generation, but using delegate_to_child instead.
- Children used read_file + replace_text (not re-generation) because
  they have their full conversation history and know their code.
- tsc passes after modification — type safety maintained across edits.
- The architect correctly identified which children needed changes
  (all components for dark mode) and updated shared types itself.
- This is the core loop: generate -> validate -> modify -> validate.
  All within the same persistent agent tree.
"""
...
def main():
    ...
    project_root = tempfile.mkdtemp(prefix="morph_exp021_")

    print("=" * 60)
    print("PERSISTENT AGENT TREE EXPERIMENT")
    print("=" * 60)
