"""
Two-Wave Fork Experiment

Improvement over experiment015: the root doesn't write types/mock data itself.
Instead it forks in two waves:
  Wave 1: Fork children for types + mock data. Wait for them.
  Wave 2: Fork children for all components + hook + App. (They can now
           read the types/mock data that wave 1 wrote to disk.)

This means the root does NO file writing — it's pure planning + forking.
The hypothesis is that the root finishes faster (fewer API calls) and
wave 2 children can read actual files on disk rather than relying solely
on inherited conversation context for type info.

## Experiment Results (2026-03-27)

Model: claude-sonnet-4-6
Root task: Product dashboard.
9 agents, 27 API calls, 10K tokens + 92K cache reads.

### Timing Summary

    Total wall clock:     82s
    Total API calls:      27
    Total tokens:         10,369 (1.5K in, 8.9K out)
    Cache read tokens:    91,653

### Agent Timings

    architect          81.5s  (43s API, 39s waiting, 7 calls)
    mock-data-child    21.6s  (2 calls — wave 1 bottleneck)
    app-child          21.7s  (2 calls)
    searchbar-child    18.9s  (3 calls)
    spinner-child      16.3s  (3 calls)
    productcard-child  14.3s  (3 calls)
    sortcontrols       11.5s  (3 calls)
    hook-child         10.4s  (2 calls)
    types-child         5.2s  (2 calls)

### Key Findings

- Two waves made root API time faster (43s vs 45-62s) because it does
  zero file writing — pure planning and forking.
- BUT wall clock is SLOWER (82s vs 54s in exp015) because of sequential
  wait between waves. mock-data-child took 22s, blocking wave 2.
- The two-wave pattern is unnecessary for this architecture. Children
  inherit the parent's conversation context which includes the type
  definitions the root discussed. They don't need to read types from
  disk — they already know them from the fork.
- The sequential dependency between waves adds ~20s of idle time that
  exp015's single-wave fork avoids entirely.
- Conclusion: exp015's single-wave fork is the optimal pattern. Two waves
  would only help if children couldn't rely on inherited context and
  needed to read files from disk, which forking makes unnecessary.
- Over-engineering the dependency graph hurt performance for this task size.
"""
...
def main():
    ...
    root_message = (
        f"{root_task}\n\n"
        "You are the architect. Don't write any code yourself.\n"
        "Work in TWO WAVES:\n\n"
        "WAVE 1 — Foundation:\n"
        "1. Plan the project structure — decide what components, hooks, types, and utils you need.\n"
        "2. Fork a child for src/types/index.ts (shared TypeScript types).\n"
        "3. Fork a child for src/utils/mockProducts.ts (mock product data).\n"
        "4. wait_all() — these must finish before wave 2 so components can read them.\n\n"
        "WAVE 2 — Components:\n"
        "5. Fork a child for EACH component (SearchBar, SortControls, ProductCard, LoadingSpinner, etc.).\n"
        "6. Fork a child for the custom hook (useProducts).\n"
        "7. Fork a child for App.tsx.\n"
        "8. wait_all(), then done().\n\n"
        "Keep fork tasks short (2-3 sentences). Children inherit your full context."
    )

    project_root = tempfile.mkdtemp(prefix="morph_exp018_")
    total_start = time.monotonic()

    print("=" * 60)
    print("TWO-WAVE FORK EXPERIMENT")
    print("=" * 60)
    print(f"\nRoot task: {root_task}")
